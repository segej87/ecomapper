...
func rectFromStartAndEnd(_ startPoint:CGPoint, endPoint: CGPoint, square: Bool, maxX: CGFloat, maxY: CGFloat) -> CGRect
{
    ...
    var heightOut = bottom-top
    var widthOut = right-left

    if square {
        // Check if either dimension is out-of-bounds, then calculate height and width
        let tempTop = max(top,0)
        let tempBottom = min(bottom,maxY)
        let tempHeight = tempBottom-tempTop
        let tempLeft = max(left,0)
        let tempRight = min(right,maxX)
        let tempWidth = tempRight-tempLeft
        
        // Set the width equal to the smallest dimension, then set height equal to width
        widthOut = min(tempHeight,tempWidth)
        heightOut = widthOut
        
        // Corrections for negatively-moving drags
        if startPoint.x > endPoint.x {
            left = startPoint.x-widthOut
        }
        if startPoint.y > endPoint.y {
            top = startPoint.y-heightOut
        }
    }
    
    let result = CGRect(x: left, y: top, width: widthOut, height: heightOut)
    return result
}
...
class CroppableImageView: UIView, CornerpointClientProtocol
{
    ...
    var constrainSquare: Bool = false
    ...
    override func layoutSubviews()
    {
        ...
        if imageToCrop != nil
        {
            ...
            viewForImage.layer.contents = theImageRef
        }
    }
    ...
    func handleDragInView(_ thePanner: UIPanGestureRecognizer)
    {
        ...
        case UIGestureRecognizerState.changed:
            ...
            else
            {
                ...
                self.cropRect = rectFromStartAndEnd(startPoint!, endPoint: newPoint, square: constrainSquare, maxX: imageRect!.size.width, maxY: imageRect!.size.height)
            }
        default:
            draggingRect = false;
            break
    ...
    func handleViewTap(_ theTapper: UITapGestureRecognizer)
    {
    ...
    func cornerHasChanged(_ newCornerPoint: CornerpointView)
    {
        ...
        cropRect = rectFromStartAndEnd(newCornerPoint.centerPoint!, endPoint: cornerpoints[otherIndex].centerPoint!, square: constrainSquare, maxX: imageRect!.size.width, maxY: imageRect!.size.height)
    }
}
